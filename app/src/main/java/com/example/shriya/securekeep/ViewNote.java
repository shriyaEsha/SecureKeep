...
public class ViewNote extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final EditText cipherkeyText1 = (EditText) findViewById(R.id.cipherkey);


        //read from DB and display message, title and cipherkey
        Intent i = getIntent();
        String filename = i.getStringExtra("filename");
        Log.w("","Filename passed thru intent: "+filename);
        loginDataBaseAdapter = new LoginDataBaseAdapter(ViewNote.this);
        loginDataBaseAdapter = loginDataBaseAdapter.open();
        String keyValue = loginDataBaseAdapter.getSingleFileEntry(filename);
        notetitle1.setText(filename.substring(0,filename.length()-4));
        cipherkeyText1.setText(keyValue);
        //read File
        File sdcard = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/SecureKeep/Shriya/Notes");
...
//Read text from file
        ...
        viewnote1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent i = new Intent(ViewNote.this, ViewNotes.class);
                startActivity(i);
...
// now attach the OutputStream to the file object, instead of a String representation
                try {
                    if(title.equals(".txt"))//no title
                        Toast.makeText(ViewNote.this,"Enter title for note!",Toast.LENGTH_SHORT).show();
                    else {
                        FileOutputStream out = new FileOutputStream(file);
                        out.write(data.getBytes(), 0, data.length());
                        out.close();
                        Toast.makeText(ViewNote.this, "Wrote file " + title, Toast.LENGTH_SHORT).show();
    ...
    public void decrypt()
    {
        ...
        if (cipherkey2.equals(""))
            Toast.makeText(ViewNote.this, "Enter CipherKey", Toast.LENGTH_SHORT).show();
        else {
